...
public class ScreenDamage : MonoBehaviour
{
    ...
    public void ShowDamage(float CurrentHealth)
    {
        opacity = 1 - (CurrentHealth / maxHealth);

        var tempColor = bloodyFrame.color;
        tempColor.a = opacity;
        bloodyFrame.color = tempColor;

        StopCoroutine("HealOverTime"); // Stop any ongoing healing coroutine
        StartCoroutine("HealOverTime"); // Start the healing coroutine
    }
    ...
    IEnumerator HealOverTime()
    {
        ...
        while (opacity > 0)
        {
            ...
            float progress = elapsedTime / 2f; // 2f is the desired healing duration
            opacity = Mathf.Lerp(initialOpacity, 0, progress);
